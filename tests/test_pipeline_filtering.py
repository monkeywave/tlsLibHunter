...
class TestMacosTlsCandidateFiltering:
    ...
    def test_skips_metal(self, classifier):
        assert not classifier.is_tls_candidate("Metal", "/System/Library/Frameworks/Metal.framework/Versions/A/Metal")
    ...
    def test_skips_iokit(self, classifier):
        assert not classifier.is_tls_candidate("IOKit", "/System/Library/Frameworks/IOKit.framework/Versions/A/IOKit")
    ...
    def test_keeps_network_framework(self, classifier):
        assert classifier.is_tls_candidate("Network", "/System/Library/Frameworks/Network.framework/Versions/A/Network")
    ...
    def test_keeps_libssl(self, classifier):
        assert classifier.is_tls_candidate("libssl.48.dylib", "/usr/lib/libssl.48.dylib")

    def test_keeps_libboringssl(self, classifier):
        assert classifier.is_tls_candidate("libboringssl.dylib", "/usr/lib/libboringssl.dylib")

    def test_keeps_libcoretls(self, classifier):
        assert classifier.is_tls_candidate("libcoretls.dylib", "/usr/lib/libcoretls.dylib")

    # App-bundled libraries should ALWAYS pass
    def test_keeps_app_bundled_lib(self, classifier):
        assert classifier.is_tls_candidate("libnss3.dylib", "/Applications/Firefox.app/Contents/MacOS/libnss3.dylib")

    def test_keeps_homebrew_lib(self, classifier):
        assert classifier.is_tls_candidate("libssl.3.dylib", "/opt/homebrew/opt/openssl/lib/libssl.3.dylib")

    def test_keeps_user_installed_lib(self, classifier):
        assert classifier.is_tls_candidate("libgnutls.so.30", "/usr/local/lib/libgnutls.so.30")
...
class TestNonMacosPlatformsUnchanged:
    ...
    def test_linux_passes_all_scan_worthy(self):
        ...
        assert classifier.is_tls_candidate("libfoo.so", "/usr/lib/x86_64-linux-gnu/libfoo.so")
